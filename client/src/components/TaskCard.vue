...
<template>
  <div class="task-card glass-panel" @click="showDetail = true">
    ...
    <div class="task-actions">
      <button @click.stop="handleToggle" class="btn-icon" :title="task.completed ? 'Mark incomplete' : 'Mark complete'">
        <IconCheck v-if="task.completed" />
        <IconCircle v-else />
      </button>
      <button @click.stop="$emit('edit', task)" class="btn-icon" title="Edit task">
        <IconEdit />
      </button>
      <button @click.stop="handleDelete" class="btn-icon btn-danger-icon" title="Delete task">
        <IconTrash />
      </button>
    </div>

    <Teleport to="body">
      <TaskDetailModal :show="showDetail" :task="task" @close="showDetail = false" @update="handleUpdate" @delete="handleDelete" @edit="handleEdit" />
    </Teleport>
  </div>
</template>
